...
{
  rt_holder hold = (rt_holder)malloc(sizeof(rt_holder_t));
  rt_set_frame_pad(p, frame_size, pad_factor);
...
{
  rt_set_frame_pad(p, frame_size, p->hold->pad_factor);
}
...
void rt_set_overlap(rt_params p, rt_uint overlap_factor)
{
  if (overlap_factor > RT_OVERLAP_MAX || overlap_factor < RT_OVERLAP_MIN) {
    fprintf(stderr, "Overlap factor must be in range %lu-%lu. Got value %lu\n",
...
void rt_set_pad_factor(rt_params p, rt_uint pad_factor)
{
  if (pad_factor > RT_PAD_MAX) {
    fprintf(stderr, "Pad factor must less than %lu. Got value %lu\n",
            RT_PAD_MAX, pad_factor);
    exit(1);
  }
  rt_set_frame_pad(p, p->hold->frame_size, pad_factor);
}
...
void rt_set_frame_pad(rt_params p, rt_uint frame_size, rt_uint pad_factor)
{
  ...
  rt_uint fft_size = 1UL << (frame_pow + pad_factor);
  if (p->fft_size > p->fft_max) {
    fprintf(stderr, "Exceeded maximum frame size.\n");
    exit(1);
  }
  else if (p->fft_size < p->fft_min) {
