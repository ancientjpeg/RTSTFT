...
const rt_command_define_t *rt_parser_check_command_exists(rt_parser   parser,
                                                          const char *token)
{
  rt_uint search_pos = RT_CMD_ALL_COMMANDS_COUNT / 2, str_pos = 0;
  rt_uint depth = RT_CMD_MAX_SEARCH_DEPTH, radius;
  char    token_char, cmd_char;
  /* because unsigned, also tests for below zero case */
  while (depth >= 0) {
    while (str_pos < RT_CMD_NAME_LEN) {
  ...
  sprintf(parser->error_msg_buffer, "Failed to find token %s.", token);
  return NULL;
}
...
int rt_parser_parse_in_place(rt_parser parser)
{
  ...
  while ((token = parser->token_buffer + token_index)->token_flavor
    ...
    else if (!(token->token_flavor
               & parser->active_cmd_def->cmd_argtypes[parsed_cmd_args])) {
      sprintf(parser->error_msg_buffer,
